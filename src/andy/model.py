...
def optimize_hyperparameters(X, y, preprocessor, n_trials: int = 30) -> dict:
    ...
    def objective(trial):
        ...
        model = create_model_pipeline(
            preprocessor,
            RandomForestClassifier(**params, class_weight="balanced", random_state=42),
        )
...
def select_features(
    X_processed, y, model: BaseEstimator, feature_names: list[str]
) -> tuple[list[str], RFECV]:
    """
    Applies RFECV to select the most important features.
    """
    selector = RFECV(
        estimator=model, step=1, scoring="roc_auc", cv=3, n_jobs=-1, verbose=1
    )
...
def explain_with_shap(model: BaseEstimator, X_selected, feature_names: list[str]):
    ...
    shap.summary_plot(
        shap_values[1], X_selected, feature_names=feature_names, show=True
    )


def final_modeling_pipeline(
    X_processed, y, feature_names, selected_features, eda_features, best_params
):
    # Combine features
...
def evaluate_features_by_count(
    model, X, y, feature_names, max_features=30, step=5, verbose=True
):
    scores = []
